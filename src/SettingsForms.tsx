...
export function FeedbackDialog({ close }: { close: () => void }) {
  ...
  async function send() {
    ...
    try { await request(`${API_BASE}/api/feedback`, { content }); setContent(""); setMessage("已发送并保存"); }
...
export function AccountSettingsForm({ action, close, onPhone }: { action: string; close: () => void; onPhone: (value: string) => void }) {
  ...
  useEffect(()=>{ if(action === "verify") request(`${API_BASE}/api/account/settings`).then(d=>setMasked(d.identityMasked)).catch(e=>setMessage(e.message)); },[action]);
  ...
  async function run(sending = false) {
    ...
    try {
      if(action === "phone") {
        if(sending) { const data=await request(`${API_BASE}/api/auth/sms/send`,{phone}); setCountdown(data.retryAfter || 60); setMessage("验证码已发送"); }
        else { const data=await request(`${API_BASE}/api/auth/sms/verify`,{phone,code,action:"bind-phone"}); onPhone(data.phone); setCode(""); setMessage("手机号已更新"); }
      } else {
        const data=await request(`${API_BASE}/api/account/settings`,{action:"identity",name,identity});
...
export function MembershipPreferences() {
  ...
  useEffect(()=>{if(!open)return; setLoaded(false); setMessage(""); request(`${API_BASE}/api/account/settings`).then(d=>{setType(d.templateType);setStyle(d.templateStyle);setLoaded(true);}).catch(e=>setMessage(e.message));},[open]);
  async function save() {setBusy(true);setMessage("");try{await request(`${API_BASE}/api/account/settings`,{action:"preferences",templateType:type,templateStyle:style});setMessage("已保存");}catch(e){setMessage((e as Error).message);}finally{setBusy(false);}}
